...
class Role(db.Model):
    ...
    name = db.Column(db.String(16),unique=True)#设置名称，长度16字节
    #与User关联，取为users，要在关系的另一个模型中添加反向应用backref
...
class User(db.Model):
    ...
    name = db.Column(db.String(16),unique=True)#用户名不能重复
    email=db.Column(db.String(32),unique=True)
    password=db.Column(db.String(32))
    #db.ForeignKey('roles.id')调用外键（表名.id）
